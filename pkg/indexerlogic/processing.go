...
// subForumNames would be map[string]string from ReadSubForumListCSV (SubForumID -> SubForumName).
func ProcessTopicsAndSubForums(allTopics []data.Topic, subForumNames map[string]string) ([]data.SubForum, error) {
	...
	for sfID, sfTopics := range topicsBySubForum {
		sfName, ok := subForumNames[sfID]
		if !ok {
			log.Printf("[WARNING] ProcessTopicsAndSubForums: SubForumID '%s' found in topic data but not in subforum list. Using ID as name.", sfID)
			sfName = sfID // Use ID as name if not found
		}
...
func LoadAndProcessTopicIndex(cfg *config.Config) (data.MasterTopicList, error) {
	log.Println("[INFO] Starting Topic Index Consumption and Prioritization Logic...")

	subForumNames, err := ReadSubForumListCSV(cfg.SubForumListFile)
	if err != nil {
		log.Printf("[FATAL] Failed to load subforum list from %s: %v", cfg.SubForumListFile, err)
		return data.MasterTopicList{}, fmt.Errorf("LoadAndProcessTopicIndex: failed to load subforum list: %w", err)
	}
	log.Printf("[INFO] Successfully loaded %d entries from subforum list: %s", len(subForumNames), cfg.SubForumListFile)
	...
	if err != nil {
		...
		return data.MasterTopicList{}, fmt.Errorf("LoadAndProcessTopicIndex: failed to read topic index directory: %w", err)
	}
	...
	subForums, err := ProcessTopicsAndSubForums(allTopics, subForumNames)
	if err != nil {
		...
		return data.MasterTopicList{}, fmt.Errorf("LoadAndProcessTopicIndex: failed to process topics: %w", err)
	}
