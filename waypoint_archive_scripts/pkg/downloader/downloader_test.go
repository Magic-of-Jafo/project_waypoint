...
func TestFetchPage_PolitenessDelay(t *testing.T) {
	...
	if requestDuration < delay {
		t.Errorf("Expected request duration >= %v (politeness delay), got %v", delay, requestDuration)
	}

	// Also check it's not excessively long (e.g. delay + reasonable processing time)
	if requestDuration > delay+(200*time.Millisecond) { // Allowing generous 200ms for test overhead
		t.Errorf("Request duration %v significantly exceeded expected %v", requestDuration, delay)
	}
}

func TestFetchPage_NoUserAgent(t *testing.T) {
	server := newMockServer(func(w http.ResponseWriter, r *http.Request) {
		if ua := r.Header.Get("User-Agent"); ua != "" {
			t.Errorf("Expected empty User-Agent, got '%s'", ua)
		}
		fmt.Fprint(w, "OK")
	})
	defer server.Close()
	...
	cfg := newTestConfig()
	cfg.UserAgent = "" // Explicitly set no User-Agent
	d := NewDownloader(cfg)
...
func TestNewDownloader(t *testing.T) {
	cfg := &config.Config{
		...
		PolitenessDelay: 50 * time.Millisecond,
	}
	...
	} else if d.Client.Timeout != 30*time.Second {
...
func TestHTTPError_ErrorMethod(t *testing.T) {
	...
	expectedMsg := "HTTP error 403 fetching URL http://example.com/forbidden"
