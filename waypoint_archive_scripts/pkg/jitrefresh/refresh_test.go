...
import (
	...
	// "waypoint_archive_scripts/pkg/htmlutil" // No longer needed to import for direct patching
)
...
func mockUtilExtractTopicsFromHTMLInUtil(htmlContent string, pageURL string, subForumID string) ([]data.Topic, error) {
	...
	doc.Find("a[href*='topic.php?id=']").Each(func(i int, s *goquery.Selection) {
		...
		if topicID != "" {
			topics = append(topics, data.Topic{ID: topicID, SubForumID: subForumID, Title: title, URL: href}) // Store full href as URL for now
		}
	})
...
func TestPerformJITRefresh(t *testing.T) {
	// No longer need to save/restore original htmlutil functions as they are not globally patched.

	// originalLogOutput := log.Writer() // Keep for debugging if needed
	...
	tests := []struct {
		...
		mockExtractErrOnURL string // Added this field
		wantNewTopics       []data.Topic
		wantErr             bool
		expectedLogs        []string
	}{
		// Test cases remain largely the same, but PerformJITRefresh will be called differently
		// --- Scenario 1: Successful discovery of new topics ---
		{
			...
			subForumData: data.SubForum{
				...
				Topics: []data.Topic{
					{ID: "t100", SubForumID: "sf1", Title: "Existing Topic 100"},
				},
			},
			cfg: &config.Config{
				JITRefreshPages: 2,
			...
			wantNewTopics: []data.Topic{
				{ID: "t101", SubForumID: "sf1", Title: "New Topic 101", URL: "topic.php?id=t101"},
				{ID: "t102", SubForumID: "sf1", Title: "New Topic 102 From Page 2", URL: "topic.php?id=t102"},
			},
			wantErr: false,
			expectedLogs: []string{
				"[INFO] JIT REFRESH: Starting for SubForum: SubForum One (ID: sf1, URL: http://forum.example.com/sf1), JITRefreshPages: 2",
				...
				"[TEST-MOCK] mockUtilExtractTopicsFromHTMLInUtil for http://forum.example.com/sf1, subForumID sf1, extracted 2 topics",
				...
				"[DEBUG] JIT REFRESH: Scanning paginated page 1/1 (URL: http://forum.example.com/sf1?page=2) for sub-forum sf1",
				"[TEST-MOCK] mockUtilFetchHTML called for URL: http://forum.example.com/sf1?page=2",
				"[TEST-MOCK] mockUtilExtractTopicsFromHTMLInUtil for http://forum.example.com/sf1?page=2, subForumID sf1, extracted 1 topics",
				"[DEBUG] JIT REFRESH: Found 1 topics on page http://forum.example.com/sf1?page=2",
				"[INFO] JIT REFRESH: Discovered new topic for sf1: ID t101, Title: New Topic 101",
				"[INFO] JIT REFRESH: Discovered new topic for sf1: ID t102, Title: New Topic 102 From Page 2",
				"[INFO] JIT REFRESH: Completed for SubForum sf1. Discovered 2 new topics from 2 scanned pages.",
			},
		},
		// --- Scenario 2: No new topics found ---
		{
			...
			subForumData: data.SubForum{
				...
				Topics: []data.Topic{{ID: "t200", SubForumID: "sf2", Title: "Existing Topic 200 Link"}},
			},
			...
			wantErr:       false,
			expectedLogs: []string{
				"[INFO] JIT REFRESH: Starting for SubForum: SubForum Two (ID: sf2, URL: http://forum.example.com/sf2), JITRefreshPages: 1",
				...
				"[TEST-MOCK] mockUtilExtractTopicsFromHTMLInUtil for http://forum.example.com/sf2, subForumID sf2, extracted 1 topics",
				"[DEBUG] JIT REFRESH: Found 1 topics on initial page http://forum.example.com/sf2",
				// "[DEBUG] JIT REFRESH: Parsing pagination links from initial page HTML of http://forum.example.com/sf2", // This line should be commented
				// "[TEST-MOCK] mockUtilParsePaginationLinks for http://forum.example.com/sf2, parsed 0 unique links from HTML content.", // This line should be commented
				"[INFO] JIT REFRESH: Reached JITRefreshPages limit (1) after processing initial page for sub-forum sf2. Stopping scan.",
				"[INFO] JIT REFRESH: Completed for SubForum sf2. Discovered 0 new topics from 1 scanned pages.",
			},
		},
		// --- Scenario 3: Sub-forum URL is empty ---
		{
			name:          "sub-forum URL empty",
			subForumData:  data.SubForum{ID: "sf3", Name: "SubForum Three", URL: ""},
			cfg:           &config.Config{JITRefreshPages: 1, UserAgent: "TestAgent/1.0"},
			wantNewTopics: []data.Topic{},
			wantErr:       false,
			expectedLogs:  []string{"[WARNING] JIT REFRESH: SubForum SubForum Three (ID: sf3) has no URL. Skipping JIT refresh."},
		},
		// --- Scenario 4: JITRefreshPages is 0 ---
		{
			name:         "JITRefreshPages is 0",
			subForumData: data.SubForum{ID: "sf4", Name: "SubForum Four", URL: "http://forum.example.com/sf4"},
			cfg:          &config.Config{JITRefreshPages: 0, PolitenessDelay: 0, ForumBaseURL: "http://forum.example.com", UserAgent: "TestAgent/1.0"},
			mockHTTPResponses: map[string]string{
				// This response should not be used if JITRefreshPages is 0 due to the early exit
				"http://forum.example.com/sf4": `<html><body><a href="topic.php?id=t400">Topic 400</a></body></html>`,
			},
			wantNewTopics: []data.Topic{},
			wantErr:       false,
			expectedLogs:  []string{"[INFO] JIT REFRESH: JITRefreshPages is 0 for SubForum sf4. Skipping JIT scan."},
		},
		// --- Scenario 5: Error fetching initial page for pagination ---
		{
			name:          "error fetching initial page",
			subForumData:  data.SubForum{ID: "sf5", Name: "SubForum Five", URL: "http://forum.example.com/sf5_fetch_error"},
		...
		// --- Scenario 6: Error parsing pagination links ---
		{
			...
			subForumData: data.SubForum{ID: "sf6", Name: "SubForum Six", URL: "http://forum.example.com/sf6_parse_error"},
			cfg:          &config.Config{JITRefreshPages: 1, PolitenessDelay: 0, UserAgent: "TestAgent/1.0"},
			mockHTTPResponses: map[string]string{
			...
			wantNewTopics: []data.Topic{{ID: "t600", SubForumID: "sf6", Title: "Topic 600", URL: "topic.php?id=t600"}},
			wantErr:       false,
			expectedLogs: []string{
				"[INFO] JIT REFRESH: Starting for SubForum: SubForum Six (ID: sf6, URL: http://forum.example.com/sf6_parse_error), JITRefreshPages: 1",
				...
				"[TEST-MOCK] mockUtilExtractTopicsFromHTMLInUtil for http://forum.example.com/sf6_parse_error, subForumID sf6, extracted 1 topics",
				"[DEBUG] JIT REFRESH: Found 1 topics on initial page http://forum.example.com/sf6_parse_error",
				"[INFO] JIT REFRESH: Reached JITRefreshPages limit (1) after processing initial page for sub-forum sf6. Stopping scan.",
				"[INFO] JIT REFRESH: Discovered new topic for sf6: ID t600, Title: Topic 600",
				"[INFO] JIT REFRESH: Completed for SubForum sf6. Discovered 1 new topics from 1 scanned pages.",
			},
		},
		// --- Scenario 7: Error fetching a subsequent page ---
		{
			name:         "error fetching subsequent page",
			subForumData: data.SubForum{ID: "sf7", Name: "SF Seven", URL: "http://forum.example.com/sf7"},
			...
			wantNewTopics: []data.Topic{{ID: "t700", SubForumID: "sf7", Title: "Topic 700", URL: "topic.php?id=t700"}},
			wantErr:       false,
			expectedLogs: []string{
				"[INFO] JIT REFRESH: Starting for SubForum: SF Seven (ID: sf7, URL: http://forum.example.com/sf7), JITRefreshPages: 2",
				...
				"[TEST-MOCK] mockUtilExtractTopicsFromHTMLInUtil for http://forum.example.com/sf7, subForumID sf7, extracted 1 topics",
				...
				"[DEBUG] JIT REFRESH: Found 1 pagination links. Processing them.",
				"[DEBUG] JIT REFRESH: Scanning paginated page 1/1 (URL: http://forum.example.com/sf7?page=2_fetch_error) for sub-forum sf7",
				...
				"[WARNING] JIT REFRESH: Failed to fetch page http://forum.example.com/sf7?page=2_fetch_error for sub-forum sf7: mockUtilFetchHTML: no response for http://forum.example.com/sf7?page=2_fetch_error. Skipping page.",
				"[INFO] JIT REFRESH: Discovered new topic for sf7: ID t700, Title: Topic 700",
				"[INFO] JIT REFRESH: Completed for SubForum sf7. Discovered 1 new topics from 2 scanned pages.",
			},
		},
		// --- Scenario 8: Error extracting topics from a subsequent page ---
		{
			name:         "error extracting topics from subsequent page",
			subForumData: data.SubForum{ID: "sf8_sub", Name: "SF Eight Sub", URL: "http://forum.example.com/sf8_sub"},
			cfg:          &config.Config{JITRefreshPages: 2, PolitenessDelay: 0, UserAgent: "TestAgent/1.0"},
			mockHTTPResponses: map[string]string{
			...
			wantNewTopics:       []data.Topic{{ID: "t800", SubForumID: "sf8_sub", Title: "Topic 800", URL: "topic.php?id=t800"}},
			wantErr:             false,
			expectedLogs: []string{
				"[INFO] JIT REFRESH: Starting for SubForum: SF Eight Sub (ID: sf8_sub, URL: http://forum.example.com/sf8_sub), JITRefreshPages: 2",
				...
				"[TEST-MOCK] mockUtilExtractTopicsFromHTMLInUtil for http://forum.example.com/sf8_sub, subForumID sf8_sub, extracted 1 topics",
				...
				"[DEBUG] JIT REFRESH: Found 1 pagination links. Processing them.",
				"[DEBUG] JIT REFRESH: Scanning paginated page 1/1 (URL: http://forum.example.com/sf8_sub?page=2_extract_error) for sub-forum sf8_sub",
				"[TEST-MOCK] mockUtilFetchHTML called for URL: http://forum.example.com/sf8_sub?page=2_extract_error",
				"[TEST-MOCK] mockUtilExtractTopicsFromHTMLInUtil for http://forum.example.com/sf8_sub?page=2_extract_error, subForumID sf8_sub, mockUtilExtractTopicsError: simulated extract topics error, mockUtilExtractTopicsErrorOnURL: http://forum.example.com/sf8_sub?page=2_extract_error",
				...
				"[WARNING] JIT REFRESH: Failed to extract topics from page http://forum.example.com/sf8_sub?page=2_extract_error for sub-forum sf8_sub: simulated extract topics error. Skipping page.",
				"[INFO] JIT REFRESH: Discovered new topic for sf8_sub: ID t800, Title: Topic 800",
				"[INFO] JIT REFRESH: Completed for SubForum sf8_sub. Discovered 1 new topics from 2 scanned pages.",
			},
		},
		// --- Scenario 9: Single-page sub-forum ---
		{
			name:         "single-page sub-forum",
			subForumData: data.SubForum{ID: "sf9", Name: "SubForum Nine", URL: "http://forum.example.com/sf9_single"},
			cfg:          &config.Config{JITRefreshPages: 1, PolitenessDelay: 0, UserAgent: "TestAgent/1.0"},
			mockHTTPResponses: map[string]string{
			...
			wantNewTopics: []data.Topic{
				{ID: "t901", SubForumID: "sf9", Title: "SP Topic 901", URL: "topic.php?id=t901"},
				{ID: "t902", SubForumID: "sf9", Title: "SP Topic 902", URL: "topic.php?id=t902"},
			},
			wantErr: false,
			expectedLogs: []string{
				"[INFO] JIT REFRESH: Starting for SubForum: SubForum Nine (ID: sf9, URL: http://forum.example.com/sf9_single), JITRefreshPages: 1",
				...
				"[TEST-MOCK] mockUtilExtractTopicsFromHTMLInUtil for http://forum.example.com/sf9_single, subForumID sf9, extracted 2 topics",
				"[DEBUG] JIT REFRESH: Found 2 topics on initial page http://forum.example.com/sf9_single",
				// "[DEBUG] JIT REFRESH: Parsing pagination links from initial page HTML of http://forum.example.com/sf9_single", // This line should be commented
				// "[TEST-MOCK] mockUtilParsePaginationLinks for http://forum.example.com/sf9_single, parsed 0 unique links from HTML content.", // This line should be commented
				"[INFO] JIT REFRESH: Reached JITRefreshPages limit (1) after processing initial page for sub-forum sf9. Stopping scan.",
				"[INFO] JIT REFRESH: Discovered new topic for sf9: ID t901, Title: SP Topic 901",
				"[INFO] JIT REFRESH: Discovered new topic for sf9: ID t902, Title: SP Topic 902",
				"[INFO] JIT REFRESH: Completed for SubForum sf9. Discovered 2 new topics from 1 scanned pages.",
			},
		},
		// --- Scenario 10: UserAgent passed to FetchHTML ---
		{
			name:         "UserAgent passed to FetchHTML",
			subForumData: data.SubForum{ID: "sfUA", Name: "SubForum UserAgent", URL: "http://forum.example.com/sfUA"},
			...
			wantNewTopics: []data.Topic{
				{ID: "ua1", SubForumID: "sfUA", Title: "UA Topic 1", URL: "topic.php?id=ua1"},
			},
			wantErr: false,
			expectedLogs: []string{
				"[INFO] JIT REFRESH: Starting for SubForum: SubForum UserAgent (ID: sfUA, URL: http://forum.example.com/sfUA), JITRefreshPages: 1",
				...
				"[TEST-MOCK] mockUtilExtractTopicsFromHTMLInUtil for http://forum.example.com/sfUA, subForumID sfUA, extracted 1 topics",
				"[DEBUG] JIT REFRESH: Found 1 topics on initial page http://forum.example.com/sfUA",
				// "[DEBUG] JIT REFRESH: Parsing pagination links from initial page HTML of http://forum.example.com/sfUA", // This line should be commented
				// "[TEST-MOCK] mockUtilParsePaginationLinks for http://forum.example.com/sfUA, parsed 0 unique links from HTML content.", // This line should be commented
				"[INFO] JIT REFRESH: Reached JITRefreshPages limit (1) after processing initial page for sub-forum sfUA. Stopping scan.",
				"[INFO] JIT REFRESH: Discovered new topic for sfUA: ID ua1, Title: UA Topic 1",
				"[INFO] JIT REFRESH: Completed for SubForum sfUA. Discovered 1 new topics from 1 scanned pages.",
			},
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Call PerformJITRefresh with the mock functions
			gotNewTopics, err := PerformJITRefresh(tt.subForumData, tt.cfg,
				mockUtilFetchHTML,
				mockUtilParsePaginationLinks,
				mockUtilExtractTopicsFromHTMLInUtil,
			)
			...
			// Explicitly check if gotNewTopics is nil BEFORE sorting
			if gotNewTopics == nil && tt.wantNewTopics != nil { // tt.wantNewTopics for "no_new_topics_found" is []data.Topic{}
